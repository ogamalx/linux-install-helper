...
- Provide `--expected-sha256` to ensure you are booting an official image instead of an unofficial or repackaged recovery (e.g., stray TWRP builds).
- Downloads whose name or URL looks like TWRP are blocked unless an expected hash is provided to avoid staging unsigned images.
- Existing TWRP-named files must be replaced with a hashed download via `--force`.
- `verify-iso` refuses to process a TWRP-named image unless you supply the vendor hash.
